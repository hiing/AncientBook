...
- 面向 Windows 用户，下载 zip 后即可运行桌面版。
- 支持多种常见文档输入，不只限于 `.txt`。
- 输出文件夹一选即可，软件自动生成安全 PDF 文件名，避免误覆盖。
- 提供古籍风格竖排排版、纸张纹理、朱栏边框和页面预览。
- 使用系统已安装中文字体，也允许用户选择自己有权使用的字体文件。
- 本机处理文档，不上传用户内容。
- 发布包提供 SHA-256 校验值，方便确认下载文件完整性。

## 适合场景

- 把文章、读书笔记、Markdown 文稿转换成古籍风格 PDF。
- 为公众号、课程资料、展示材料生成更有传统书卷感的版式。
- 在 Windows 上体验类似古籍竖排排版工具的工作流。
